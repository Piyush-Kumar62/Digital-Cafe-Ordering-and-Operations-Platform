...
export const authInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  const isRoleRoute =
    ...
    router.url?.startsWith("/owner") ||
    router.url?.startsWith("/chef") ||
    router.url?.startsWith("/waiter") ||
    router.url?.startsWith("/customer");
  const isReadRequest = req.method.toUpperCase() === "GET";
  const skipLoading =
    req.headers.has("x-silent-loading") || (isRoleRoute && isReadRequest);
  if (!skipLoading) {
    loadingService.show();
  }
