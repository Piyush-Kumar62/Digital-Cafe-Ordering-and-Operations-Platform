...
export class ApiService {
  ...
  return this.http
    .get<{ data: Cafe }>(`${this.baseUrl}/cafes/my-cafe`)
  ...
  createCafeSetup(formData: FormData) {
  return this.http.post(`${environment.apiUrl}/cafes/setup`, formData);
}
  ...
  return this.http.post(`${environment.apiUrl}/staff`, payload);
}

updateStaff(id: number, payload: any) {
  return this.http.put(`${this.baseUrl}/staff/${id}`, payload);
}
getStaffByCafe(cafeId: number) {
  return this.http.get<any[]>(`${environment.apiUrl}/staff/cafe/${cafeId}`);
}

activateStaff(id: number) {
  return this.http.patch(`${environment.apiUrl}/staff/${id}/activate`, {});
}

deactivateStaff(id: number) {
  return this.http.patch(`${environment.apiUrl}/staff/${id}/deactivate`, {});
}
  ...
  // ============ cafe Settings APIs ============
  uploadLogo(cafeId: number, formData: FormData) {
  return this.http.post(
    `http://localhost:8080/api/cafes/${cafeId}/logo`,
    formData
  );
}

uploadCover(cafeId: number, formData: FormData) {
  return this.http.post(
    `http://localhost:8080/api/cafes/${cafeId}/cover`,
    formData
  );
}

uploadGallery(cafeId: number, formData: FormData) {
  return this.http.post(
    `http://localhost:8080/api/cafes/${cafeId}/gallery`,
    formData
  );
}

//update cafe from the settings 
updateCafeSetup(id: number, formData: FormData) {
  return this.http.put(`${environment.apiUrl}/cafes/${id}`, formData);
}

// uploadCafeLogo(id: number, file: File) {
//   const form = new FormData();
//   form.append("file", file);
//   return this.http.post(`/api/cafes/${id}/logo`, form);
// }

// uploadCafeCover(cafeId: number, file: File) {
...
// uploadCafeGallery(id: number, files: FileList) {
//   const form = new FormData();
//   Array.from(files).forEach(f => form.append("files", f));
//   return this.http.post(`/api/cafes/${id}/gallery`, form);
// }
}
