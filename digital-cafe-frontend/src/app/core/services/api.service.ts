...
import { Cafe, CreateCafeRequest, Table, MenuItem, MenuItemRequest } from '@shared/models/cafe.model';
import { Order, OrderRequest, Booking, BookingRequest } from '@shared/models/order.model';
import { Payment, PaymentRequest } from '@shared/models/payment.model';
import { Profile, ProfileUpdateRequest, AcademicInfo, WorkExperience } from '@shared/models/profile.model';
import { AdminDashboard, OwnerDashboard, ChefDashboard, WaiterDashboard } from '@shared/models/dashboard.model';
...
export class ApiService {
  ...
  getAvailableTablesByCafe(cafeId: number): Observable<Table[]> {
    return this.http.get<Table[]>(`${this.baseUrl}/tables/cafe/${cafeId}/available`);
  }

  createTable(cafeId: number, table: Partial<Table>): Observable<Table> {
    return this.http.post<Table>(`${this.baseUrl}/tables/cafe/${cafeId}`, table);
  }
  ...
  getMenuItemsByCafe(cafeId: number): Observable<MenuItem[]> {
    return this.http.get<MenuItem[]>(`${this.baseUrl}/menu-items/cafe/${cafeId}`);
  }

  getAvailableMenuItemsByCafe(cafeId: number): Observable<MenuItem[]> {
    return this.http.get<MenuItem[]>(`${this.baseUrl}/menu-items/cafe/${cafeId}/available`);
  }

  getMenuItemsByCategory(cafeId: number, category: string): Observable<MenuItem[]> {
    return this.http.get<MenuItem[]>(`${this.baseUrl}/menu-items/cafe/${cafeId}/category/${category}`);
  }

  createMenuItem(cafeId: number, request: MenuItemRequest): Observable<MenuItem> {
    return this.http.post<MenuItem>(`${this.baseUrl}/menu-items/cafe/${cafeId}`, request);
  }
  ...
  deleteMenuItem(id: number): Observable<MessageResponse> {
    return this.http.delete<MessageResponse>(`${this.baseUrl}/menu-items/${id}`);
  }
  ...
  getOrdersByStatus(cafeId: number, status: string): Observable<Order[]> {
    return this.http.get<Order[]>(`${this.baseUrl}/orders/cafe/${cafeId}/status/${status}`);
  }
  ...
  updateProfile(userId: number, request: ProfileUpdateRequest): Observable<Profile> {
    return this.http.put<Profile>(`${this.baseUrl}/profiles/${userId}`, request);
  }

  addAcademicInfo(userId: number, info: AcademicInfo): Observable<Profile> {
    return this.http.post<Profile>(`${this.baseUrl}/profiles/${userId}/academic`, info);
  }

  addWorkExperience(userId: number, experience: WorkExperience): Observable<Profile> {
    return this.http.post<Profile>(`${this.baseUrl}/profiles/${userId}/work-experience`, experience);
  }

  // ============ Dashboard APIs ============
  getAdminDashboard(): Observable<AdminDashboard> {
    return this.http.get<AdminDashboard>(`${this.baseUrl}/dashboard/admin`);
  }

  getOwnerDashboard(cafeId: number): Observable<OwnerDashboard> {
    return this.http.get<OwnerDashboard>(`${this.baseUrl}/dashboard/owner/${cafeId}`);
  }

  getChefDashboard(cafeId: number): Observable<ChefDashboard> {
    return this.http.get<ChefDashboard>(`${this.baseUrl}/dashboard/chef/${cafeId}`);
  }

  getWaiterDashboard(cafeId: number): Observable<WaiterDashboard> {
    return this.http.get<WaiterDashboard>(`${this.baseUrl}/dashboard/waiter/${cafeId}`);
  }
  ...
  createCafeOwner(request: any): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/users/create-cafe-owner`, request);
  }
  ...
  toggleUserStatus(id: number): Observable<MessageResponse> {
    return this.http.put<MessageResponse>(`${this.baseUrl}/users/${id}/toggle-status`, null);
  }
}
