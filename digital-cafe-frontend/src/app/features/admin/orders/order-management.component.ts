...
loadCafes(): void {
  this.apiService.getAdminCafes(0, 200).subscribe({
    next: (res: any) => {
      ...
      console.log("Cafes API Raw Response:", res); // optional debug
  ...
  this.apiService.getCafeOrdersForAdmin(this.selectedCafeId, 0, 200).subscribe({
    next: (res: any) => {
      ...
      console.log("Orders API Raw Response:", res); // 🔍 keep for debugging

      // ✅ Normalize response into array
      let extracted: Order[] = [];

      if (Array.isArray(res)) {
        // Case: API returns direct array
        extracted = res;
      } else if (Array.isArray(res?.content)) {
        // Spring Page<Order>
        extracted = res.content;
        ...
        // Nested wrapper
        extracted = res.data.content;
      } else {
        console.warn("Unexpected orders response format", res);
        extracted = [];
      }

      this.orders = extracted;

      // Preserve selected status values
      this.orders.forEach((o) => {
        if (!this.statusMap[o.id]) {
          this.statusMap[o.id] = o.status;
        }
      });
