...
interface StaffBreakdown { chefs: number; waiters: number; others: number; }
interface BookingItem {
  id: number; customerName: string; date: string; time: string;
  tableNumber: string; guests: number; status: string;
}
...
export class CafeOwnerDashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  loading = true;
  refreshing = false;
  cafeId!: number;
  cafeName = '';
  ...
  stats = {
    totalTables: 0, availableTables: 0, occupiedTables: 0,
    totalStaff: 0, activeStaff: 0,
    staffBreakdown: { chefs: 0, waiters: 0, others: 0 } as StaffBreakdown,
    todayOrders: 0, pendingOrders: 0, preparingOrders: 0, completedOrders: 0,
    todayRevenue: 0, weekRevenue: 0,
    todayBookings: 0, upcomingBookingsCount: 0,
  ...
  quickActions = [
    { label: 'Manage Tables',  icon: '\u{1FA91}',  route: '/owner/tables',    color: 'blue'   },
    { label: 'View Orders',    icon: '\u{1F4CB}',  route: '/owner/orders',    color: 'orange' },
    { label: 'Manage Staff',   icon: '\u{1F465}',  route: '/owner/staff',     color: 'purple' },
    { label: 'Menu Items',     icon: '\u{1F37D}\uFE0F', route: '/owner/menu', color: 'green'  },
    { label: 'Bookings',       icon: '\u{1F4C5}',  route: '/owner/bookings',  color: 'cyan'   },
    { label: 'Cafe Settings',  icon: '\u2699\uFE0F', route: '/owner/settings', color: 'gray' },
  ];
  ...
  private startPolling(): void {
    this.pollingSubscription = interval(this.POLL_INTERVAL_MS).subscribe(() => {
      if (this.cafeId) { this.silentRefresh(); }
    });
  }
  ...
  loadDashboard(isRefresh = false): void {
    if (isRefresh) { this.refreshing = true; } else { this.loading = true; }

    this.apiService.cafeExistsForOwner().subscribe({
      next: (exists) => {
        if (!exists) { this.loading = false; this.router.navigate(['/owner/setup']); return; }
        this.apiService.getMyCafe().subscribe({
          ...
          error: () => { this.loading = false; this.alertService.error('Unable to load cafe info.'); },
        });
      },
      error: () => { this.loading = false; this.alertService.error('Unable to validate cafe status.'); },
    });
  }

  fetchAllDashboardData(silent = false): void {
    forkJoin({
      owner:    this.apiService.getOwnerDashboard(this.cafeId).pipe(catchError(() => of(null))),
      tables:   this.apiService.getTablesByCafe(this.cafeId).pipe(catchError(() => of([]))),
      staff:    this.apiService.getStaffByCafe(this.cafeId).pipe(catchError(() => of([]))),
      orders:   this.apiService.getOrdersByCafe(this.cafeId).pipe(catchError(() => of([]))),
      bookings: this.apiService.getBookingsByCafe(this.cafeId).pipe(catchError(() => of([]))),
      menu:     this.apiService.getMenuItemsByCafe(this.cafeId).pipe(catchError(() => of([]))),
    }).subscribe({
      next: ({ owner, tables, staff, orders, bookings, menu }) => {
        ...
        if (this.viewInitialized) { setTimeout(() => this.renderAllCharts(), 50); }
      },
      error: () => {
        this.loading = false;
        this.refreshing = false;
        if (!silent) { this.alertService.error('Failed to load dashboard data.'); }
      },
    });
  }
  ...
  private processOwnerDashboard(data: OwnerDashboard | null): void {
    ...
    if (data.popularItems?.length) {
      this.popularItems = data.popularItems.slice(0, 5).map((p) => ({
        name: p.menuItemName, orders: p.orderCount, revenue: p.totalRevenue,
      }));
    }
    if (data.revenueData?.length) {
      this.stats.weekRevenue = data.revenueData.reduce((s, r) => s + (r.revenue ?? 0), 0);
    }
    if (data.orderStatusCounts) {
      const colorMap: Record<string, string> = {
        COMPLETED: '#22c55e', PENDING: '#f59e0b', PREPARING: '#6366f1',
        CANCELLED: '#ef4444', SERVED: '#06b6d4', IN_PROGRESS: '#8b5cf6',
      };
      this.orderStatusCounts = Object.entries(data.orderStatusCounts).map(
        ([label, count]) => ({ label, count: count as number, color: colorMap[label.toUpperCase()] ?? '#94a3b8' })
      );
    }
  }
  ...
  private processTables(tables: any[]): void {
    ...
    this.stats.occupiedTables  = this.stats.totalTables - this.stats.availableTables;
  }
  ...
  private processStaff(staff: any[]): void {
    ...
    const chefs   = staff.filter((s) => s.roles?.includes('CHEF')   || s.role === 'CHEF').length;
    const waiters = staff.filter((s) => s.roles?.includes('WAITER') || s.role === 'WAITER').length;
    this.stats.staffBreakdown = { chefs, waiters, others: staff.length - chefs - waiters };
  }

  private processOrders(orders: any[]): void {
    if (!this.stats.todayOrders)   this.stats.todayOrders   = orders.length;
    if (!this.stats.pendingOrders) this.stats.pendingOrders = orders.filter((o) => o.status === 'PENDING').length;
    this.stats.preparingOrders = orders.filter((o) => o.status === 'PREPARING').length;
    this.stats.completedOrders = orders.filter((o) => o.status === 'COMPLETED').length;
    if (!this.stats.todayRevenue) {
      this.stats.todayRevenue = orders.reduce((s, o) => s + (o.totalAmount ?? 0), 0);
    }
    if (!this.orderStatusCounts.length) {
      ...
      const colorMap: Record<string, string> = {
        COMPLETED: '#22c55e', PENDING: '#f59e0b', PREPARING: '#6366f1',
        CANCELLED: '#ef4444', SERVED: '#06b6d4',
      };
      this.orderStatusCounts = Object.entries(map).map(([label, count]) => ({
        label, count, color: colorMap[label] ?? '#94a3b8',
      }));
    }
    this.recentOrders = orders.slice(0, 8).map((o) => ({
      id: o.id, amount: o.totalAmount ?? 0, status: o.status ?? 'UNKNOWN',
      tableNumber: o.tableNumber ?? String(o.tableId ?? '-'), createdAt: o.createdAt ?? '',
    }));
  }
  ...
  private renderRevenueChart(): void {
    ...
    this.apiService.getOwnerDashboard(this.cafeId).pipe(catchError(() => of(null))).subscribe((d) => {
      let labels: string[] = [];
      let data: number[]   = [];
      if (d?.revenueData?.length) {
        ...
        for (let i = 6; i >= 0; i--) {
          ...
          labels.push(d2.toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric' }));
          data.push(0);
        }
      }
        ...
        options: {
          ...
          scales: {
            y: {
              ...
              ticks: { color: '#94a3b8', callback: (v) => `\u20B9${Number(v ?? 0).toLocaleString('en-IN')}` },
            },
  ...
  private renderOrderStatusChart(): void {
    if (!this.orderStatusChartRef?.nativeElement || !this.orderStatusCounts.length) return;
  ...
  private renderTableChart(): void {
    ...
    const chart = new Chart(this.tableChartRef.nativeElement, {
      ...
      data: {
        ...
        datasets: [{
          data: [this.stats.availableTables || 0, this.stats.occupiedTables || 0],
  ...
  private renderPopularChart(): void {
    ...
    const items = this.popularItems.length ? this.popularItems
    ...
    const chart = new Chart(this.popularChartRef.nativeElement, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#818cf8','#34d399','#fb923c','#f472b6','#38bdf8'],
          borderRadius: 8,
          borderSkipped: false,
        }],
      },
  ...
  getOrderStatusClass(status: string): string {
    const map: Record<string, string> = {
      ...
      SERVED: 'status-served', READY: 'status-served', IN_PROGRESS: 'status-preparing',
    };
  ...
  getBookingStatusClass(status: string): string {
    const map: Record<string, string> = {
      BOOKED: 'status-booked', CONFIRMED: 'status-confirmed',
      PENDING: 'status-pending', CANCELLED: 'status-cancelled', COMPLETED: 'status-completed',
    };
    return map[status?.toUpperCase()] ?? 'status-default';
  }
  ...
  formatTime(dateStr: string): string {
    ...
    return isNaN(d.getTime()) ? dateStr : d.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });
  }

  refresh(): void { this.loadDashboard(true); }
}
